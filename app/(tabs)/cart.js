...
const cart = () => {
  ...
  const { user, setOrder, setUser, fetchUserData } = useGlobalContext();
  ...
  const createOrder = () => {
    ...
    const date = new Date();
    const order = {
      deliverTo: userAddressess[clickedAddress],
      date: date,
      user: `${userData.name} - ${userData.number}`,
      products: products,
    };
  ...
  return (
    <>
      ...
      >
        <Pressable disabled={!addressPop} onPress={() => setAddressPop(false)}>
          <View
              ...
              <View className="flex-row justify-between items-center my-4 mx-6">
                ...
                <View>
                  ...
                  >
                    ...
                    <Text className="text-lg font-semibold pr-2">Add New</Text>
                  </TouchableOpacity>
                </View>
              </View>
            ) : (
