...
const deliveredOrders = () => {
  ...
  const generateExcelReport = async () => {
    ...
    try {
      ...
      worksheet.columns = [
        ...
        { header: "Cancel Date", key: "date", width: 12 },
        { header: "Cancel Reason", key: "reason", width: 12 },
      ];
  ...
  const fetchInitialData = async () => {
    setLoading(true);
    const res = await getAllCancelledOrders(limit, 0);
    if (res.status) {
      setdata(res.orders);
  ...
  const fetchLaterData = async () => {
    const res = await getAllCancelledOrders(limit, offset);
    if (res.status) {
